#!/usr/bin/python3
"""Apply the six-position IMU profile before Madgwick/EKF consumption."""
...
class ImuBiasCorrector(Node):
    def __init__(self, args):
        ...
        if not calibration:
            raise RuntimeError(
                f"{args.profile} has no imu section; run rig_calibration.py imu-six first")
        ...
        if not validation or validation.get("passed") is not True:
            raise RuntimeError(
                ...
                "Repeat imu-six with the axis-alignment checks; refusing to apply it.")
        ...
        self.get_logger().info(
            f"Applying IMU profile {args.profile}: {args.input_topic} -> {args.output_topic}")
