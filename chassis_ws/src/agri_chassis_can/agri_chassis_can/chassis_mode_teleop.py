...
class ChassisModeTeleop(Node):
    ...
    def _axis(msg: Joy, index: int) -> float:
        if 0 <= index < len(msg.axes):
            ...
            return max(-1.0, min(1.0, value)) if math.isfinite(value) else math.nan
        return 0.0
    ...
    def _auto_cmd_cb(self, msg: Twist) -> None:
        if not math.isfinite(msg.linear.x) or not math.isfinite(msg.angular.z):
            self.last_auto_cmd = Twist()
            self.last_auto_time = 0.0
            self.get_logger().error("non-finite AUTO cmd_vel rejected")
            return
        self.last_auto_cmd = Twist()
        self.last_auto_cmd.linear.x = msg.linear.x
        self.last_auto_cmd.angular.z = msg.angular.z
    ...
    def _joy_cb(self, msg: Joy) -> None:
        ...
        if not math.isfinite(self.manual_linear) or not math.isfinite(self.manual_angular):
            self.manual_linear = 0.0
            self.manual_angular = 0.0
            self.last_joy_time = 0.0
            self.get_logger().error("non-finite joystick command rejected")
            return
